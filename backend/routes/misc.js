...
router.get('/notifications', auth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM notifications WHERE user_id=$1 AND is_cleared=false ORDER BY created_at DESC LIMIT 50`,
      [req.user.id]
    );
    res.json(result.rows);
  } catch (err) {
...
router.delete('/notifications/:id', auth, async (req, res) => {
  try {
    await pool.query('UPDATE notifications SET is_cleared=true WHERE id=$1 AND user_id=$2', [req.params.id, req.user.id]);
...
router.delete('/notifications', auth, async (req, res) => {
  try {
    await pool.query('UPDATE notifications SET is_cleared=true WHERE user_id=$1', [req.user.id]);
